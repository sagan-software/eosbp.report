...
// console.log("!?!?!??!?!??!", BignumberJs, typeof BignumberJs);

// console.log("????1", new BignumberJs(0));
// console.log("????2");
...
function fetchWithUrl(url, method__, body, $staropt$star, _) {
	...
	return Request$ReactTemplate.make(
		urlStr,
		method__,
		false,
		body,
...
function fetchWithBase(baseUrl, path, method__, body, timeout, _) {
	...
	try {
		...
		return Promise.reject([BadUrl, baseUrl]);
	}
	if (exit === 1) {
...
function fetch(url, method__, body, timeout, _) {
	...
	try {
		...
		return Promise.reject([BadUrl, url]);
	}
	if (exit === 1) {
...
var InvalidJson = Caml_exceptions.create(
	"Fetch_json-ReactTemplate.InvalidJson",
);
...
function thenDecode(decoder, promise) {
	return promise.then(function(response) {
		...
		try {
			...
			return Promise.reject([InvalidJson, response]);
		}
		if (exit === 1) {
			return Promise.resolve(/* tuple */ [response, data]);
		}
	});
}

var DecodeError = Caml_exceptions.create(
	"Fetch_json-ReactTemplate.DecodeError",
);
...
function requireDecoded(promise) {
	return promise.then(function(param) {
		...
		if (match.tag) {
			return Promise.reject([DecodeError, response, match[0]]);
		} else {
			return Promise.resolve(/* tuple */ [response, data, match[0]]);
		}
	});
}
...
function tableRows(lowerBound, _) {
	return Curry._6(
		...
		Npmlog.info(
			"regproducer",
			"total=" + (String(total) + (" more=" + (String(more) + ""))),
			"",
		);
		if (more) {
			...
			if (lastRow !== undefined) {
				var nextLowerBound = new BignumberJs(
					Eosjs.modules.format.encodeName(
						Eos_Types.AccountName[/* toString */ 4](lastRow[/* owner */ 0]),
						false,
					),
				)
					.plus(1)
...
function bpJsonRaw(row) {
	...
	if (match) {
		...
		return Promise.reject([BadUrl, row[/* url */ 4]]);
	}
}
...
function producerDir(row) {
	return Path.join(
		Env$ReactTemplate.buildDir,
		Eos_Types.AccountName[/* toString */ 4](row[/* owner */ 0]),
	);
}
...
function writeBpJson(param) {
	var row = param[2];
	var writeBpRawJson = writeProducerFile(
		row,
		"bp-raw.json",
		param[0][/* text */ 5],
		/* utf8 */ -844212943,
	);
	var writeBpJson$1 = writeProducerFile(
		row,
		"bp.json",
		JSON.stringify(param[1][/* json */ 0], null, 2),
		/* utf8 */ -844212943,
	);
	return Promise.all(/* tuple */ [writeBpRawJson, writeBpJson$1]);
}
...
var Unreachable = Caml_exceptions.create(
	"Fetch_json-ReactTemplate.Unreachable",
);

var BadResponse = Caml_exceptions.create(
	"Fetch_json-ReactTemplate.BadResponse",
);
...
function handleBpJsonError(match) {
	if (Caml_exceptions.isCamlExceptionOrOpenVariant(match)) {
		if (match[0] === BadUrl) {
			...
			return (
				"Bad status: " +
				(String(match$1[/* statusCode */ 2]) +
					(" (" +
						(String(match$1[/* statusText */ 3]) +
							(") at " + (String(match$1[/* responseUrl */ 1]) + "")))))
			);
...
function fetchBpJson(row) {
	return thenDecode(EosBp_Json$ReactTemplate.decode, bpJsonRaw(row))
		.then(function(param) {
			var response = param[0];
			if (
				200 <= response[/* statusCode */ 2] &&
				response[/* statusCode */ 2] < 400
			) {
				Npmlog.info(
					"bp.json",
					Eos_Types.AccountName[/* toString */ 4](row[/* owner */ 0]),
					response[/* responseUrl */ 1],
				);
				return Promise.resolve(/* tuple */ [response, param[1], row]);
			} else {
				return Promise.reject([BadStatus, response]);
			}
		})
...
function fetchBpJsonFiles(table) {
	...
	return allChunked(table[/* rows */ 0], fetchBpJson, 25)
		.then(function(responses) {
			return Promise.resolve(withoutNone(responses));
		})
		.then(function(responses) {
			return Promise.resolve(/* tuple */ [table[/* rows */ 0], responses]);
		});
}
...
function writeBpJsonFiles(param) {
	...
	Npmlog.info(
		...
		"Got " +
			(String(numResponses) +
				(" OK responses of " + (String(numRows) + " producers"))),
		"",
	);
	return allChunked(responses, writeBpJson, 10).then(function() {
		return Promise.resolve(/* tuple */ [rows, responses]);
	});
}
...
function fetchImage(row, basename, url) {
	var url$1 = Js_option.getWithDefault("", url)
		.trim()
		.replace(/^[^\x00-\x7F]/g, "")
		.replace(/[^\x00-\x7F]$/g, "")
		.replace(/\/$/g, "");
	if (url$1.length === 0) {
		...
		return Request$ReactTemplate.make(
			...
			.then(function(res) {
				var contentType = Js_option.getWithDefault(
					"?",
					Request$ReactTemplate.header(res, "content-type"),
				);
				var match = imageExtension(contentType);
				if (match !== undefined) {
					var filename = basename + match;
					return writeProducerFile(
						row,
						filename,
						res.body,
						/* binary */ -317998079,
...
function fetchImages(row, json) {
	return Js_option.getWithDefault(
		Promise.resolve(/* array */ []),
		Js_option.map(function(branding) {
			return Promise.all(
				/* array */ [
...
function generateHtmlFile(route, dirname) {
	var element = ReasonReact.element(
		undefined,
		undefined,
		App$ReactTemplate.make(route, /* array */ []),
	);
...
function generateProducerReport(producer, globalState) {
	var estimatedRewards = Eosio_System.estimateProducerPay(
		producer,
		globalState,
	);
	return Promise.resolve(/* tuple */ [producer, estimatedRewards]);
}

function generateProducerReports(param) {
	var responses = param[1];
	var rows = param[0];
	return Curry._2(Eosio.getGlobalState(eos), undefined, /* () */ 0)
		.then(function(globalState) {
			if (globalState !== undefined) {
				var globalState$1 = globalState;
				return Promise.all(
					Belt_Array.map(rows, function(r) {
						return generateProducerReport(r, globalState$1);
					}),
				).then(function(results) {
					return Promise.resolve(
						Belt_Array.forEach(
							results.sort(function(param, param$1) {
							...
							function(param) {
								Npmlog.info(
									"rewards",
									Eos_Types.AccountName[/* toString */ 4](
										param[0][/* owner */ 0],
									),
									Eosjs.modules.format.printAsset(
										Eos_Types.Asset[/* fromBigNumber */ 0](param[1], 4, "EOS"),
									),
								);
								return /* () */ 0;
							},
...
tableRows(undefined, /* () */ 0)
	.then(fetchBpJsonFiles)
	.then(generateProducerReports)
	.then(function() {
